process OPENSWATHWORKFLOW {
  ...
  // Handle iRT files - check if file exists and is not empty list or placeholder
  def hasIrtTraml = irt_traml && !(irt_traml instanceof List && irt_traml.isEmpty()) && irt_traml.name != 'NO_IRT_FILE'
  def hasIrtNonlinear = irt_nonlinear_traml && !(irt_nonlinear_traml instanceof List && irt_nonlinear_traml.isEmpty()) && irt_nonlinear_traml.name != 'NO_IRT_FILE'
  def irtFlag = hasIrtTraml ? "-Calibration:tr_irt ${irt_traml}" : ""
  def irtFlagNonlinear = hasIrtNonlinear ? "-Calibration:tr_irt_nonlinear ${irt_nonlinear_traml}" : ""
  def swathFlag  = swath_windows && !(swath_windows instanceof List && swath_windows.isEmpty()) && swath_windows.name != 'input.3' ? "-swath_windows_file ${swath_windows}" : ""
